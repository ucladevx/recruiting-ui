			...
			<div className="cards">
				<div className="card card-wide profile-card">
					...
					<form className="app-form">
						{ Config.essays.essays.map(essay => <TextAreaInput required originalValue={this.props.profile[essay.name]} name={essay.name} key={essay.name} title={essay.title} desc={essay.desc} onChange={this.props.setValue} />) }
					</form>
				</div>
			</div>
		);
